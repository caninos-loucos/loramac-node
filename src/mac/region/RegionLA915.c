    ...
    switch( getPhy->Attribute )
    {
        ...
        case PHY_NEXT_LOWER_TX_DR:
        {
            RegionCommonGetNextLowerTxDrParams_t nextLowerTxDrParams =
            {
                ...
                .MaxDr = ( int8_t )LA915_TX_MAX_DATARATE,
                .MinDr = ( int8_t )( ( getPhy->UplinkDwellTime == 0 ) ? LA915_TX_MIN_DATARATE : LA915_DWELL_LIMIT_DATARATE ),
                .NbChannels = LA915_MAX_NB_CHANNELS,
        ...
        case PHY_BEACON_CHANNEL_FREQ:
        {
            phyParam.Value = RegionBaseUSCalcDownlinkFrequency( getPhy->Channel,
                                                                LA915_BEACON_CHANNEL_FREQ,
                                                                LA915_BEACON_CHANNEL_STEPWIDTH );
            break;
        }
        ...
        case PHY_SF_FROM_DR:
        {
            phyParam.Value = DataratesLA915[getPhy->Datarate];
            break;
        }
        case PHY_BW_FROM_DR:
        {
            phyParam.Value = RegionCommonGetBandwidth( getPhy->Datarate, BandwidthsLA915 );
            break;
        }
        default:
        {
...
void RegionLA915InitDefaults( InitDefaultsParams_t* params )
{
    ...
    switch( params->Type )
    {
        case INIT_TYPE_DEFAULTS:
        {
            ...
            // Default bands
            memcpy1( ( uint8_t* )RegionBands, ( uint8_t* )bands, sizeof( Band_t ) * LA915_MAX_NB_BANDS );
            ...
            {
                // 500 kHz channels
                RegionNvmGroup2->Channels[i].Frequency = 915900000 + ( i - ( LA915_MAX_NB_CHANNELS - 8 ) ) * 1600000;
                RegionNvmGroup2->Channels[i].DrRange.Value = ( DR_6 << 4 ) | DR_6;
                RegionNvmGroup2->Channels[i].Band = 0;
            }

            // Initialize channels default mask
            RegionNvmGroup2->ChannelsDefaultMask[0] = 0xFF;
...
void RegionLA915ComputeRxWindowParameters( int8_t datarate, uint8_t minRxSymbols, uint32_t rxError, RxConfigParams_t *rxConfigParams )
{
    ...
    rxConfigParams->Bandwidth = RegionCommonGetBandwidth( rxConfigParams->Datarate, BandwidthsLA915 );

    tSymbolInUs = RegionCommonComputeSymbolTimeLoRa( DataratesLA915[rxConfigParams->Datarate], BandwidthsLA915[rxConfigParams->Datarate] );
...
bool RegionLA915RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    Radio.SetMaxPayloadLength( MODEM_LORA, MaxPayloadOfDatarateDwell0LA915[dr] + LORAMAC_FRAME_PAYLOAD_OVERHEAD_SIZE );
...
bool RegionLA915TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    uint32_t bandwidth = RegionCommonGetBandwidth( txConfig->Datarate, BandwidthsLA915 );
    int8_t phyTxPower = 0;
...
int8_t RegionLA915NewChannelReq( NewChannelReqParams_t* newChannelReq )
{
...
int8_t RegionLA915DlChannelReq( DlChannelReqParams_t* dlChannelReq )
{
    // Do not accept the request
    return -1;
}
...
LoRaMacStatus_t RegionLA915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    countChannelsParams.MaxNbChannels = LA915_MAX_NB_CHANNELS;
    countChannelsParams.JoinChannels = NULL;

    identifyChannelsParam.AggrTimeOff = nextChanParams->AggrTimeOff;
    identifyChannelsParam.LastAggrTx = nextChanParams->LastAggrTx;
    identifyChannelsParam.DutyCycleEnabled = nextChanParams->DutyCycleEnabled;
    identifyChannelsParam.MaxBands = LA915_MAX_NB_BANDS;
    ...
    if( status == LORAMAC_STATUS_OK )
    {
        ...
        RegionCommonChanDisable( RegionNvmGroup1->ChannelsMaskRemaining, *channel, LA915_MAX_NB_CHANNELS );
    }
    return status;
}
